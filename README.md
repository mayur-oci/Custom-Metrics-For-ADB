...
4. Define PL/SQL script to populate random data of customer-orders in `SHOPPING_ORDER` table.
5. Define PL/SQL script to compute, collect/buffer & post the custom metrics *Oracle Monitoring Service*.
...
4. Read through the following PL/SQL script. It populates data in `SHOPPING_ORDER` table. Run it on Oracle DB client of your choice. </br>
...
5. Let us dive deep into actual crux of this tutorial: script which computes the custom metrics and publishes it to *Oracle Cloud Monitoring Service*. Run it on Oracle DB client of your choice.
